				...
				<?php
					while ($row = mysqli_fetch_array($result_dashboard))
					{
                        ...
                        if ($first_day != NULL)
                        {
                            ...
                            //$sql_leave = "SELECT COUNT(total_days) AS leaves FROM employee_leave_taken
                            //WHERE employee_detail_ID = $employee_detail_ID AND status = 'Approved' AND date_start >= '$period_start' AND date_end <= //'$period_end'";
