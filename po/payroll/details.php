...
if (isset($_REQUEST['srid']))
{
    ...
    $result_discrepancy = $con->query($sql_discrepancy) or die(mysqli_error($con));
    ...
    $sql_reason = "SELECT reason FROM salary_report_discrepancy WHERE salary_report_discrepancy_ID = $salary_report_discrepancy_ID";
    $result_reason = $con->query($sql_reason);
...
{
    $discrepancy = mysqli_real_escape_string($con, $_POST['discrepancy']);
    $reason = mysqli_real_escape_string($con, $_POST['reason']);
    
    $sql_discrepancy_update = "UPDATE salary_report_discrepancy SET amount = $discrepancy, date_filed = NOW(), filed_by = $employee_detail_ID WHERE salary_report_discrepancy_ID = $salary_report_discrepancy_ID";
    $con->query($sql_discrepancy_update);

    $sql_delete = "UPDATE salary_report SET status = 'Pending' WHERE salary_report_ID = $salary_report_ID"; 
...
<form method="POST" class="form-horizontal">
    <div class="col-lg-6">
        <div class="form-group">
            <label class="control-label col-lg-4">Discrepancy (amount after tax)</label>
            <div class="col-lg-4">
                <input name="discrepancy" type="number" class="form-control"  required />
            </div>
        </div>
        <div class="form-group">
            <label class="control-label col-lg-4">Reason</label>
            <div class="col-lg-4">
                <textarea name="reason" style="width:100%;height:80px;" disabled><?php echo $message ?></textarea>
            </div>
        </div>
        <div class="form-group">
